...
#Built a LIDAR detection module, with a boolean mask to filter out ego vehicle, the ground and noise points, and a DBSCAN algorithm to cluster the remaining points into objects.
# And then computer the 3D bounding boxes for each object and detect objects from point cloud.
def remove_ego(point_cloud: o3d.geometry.PointCloud):
    ...
    for i in range(len(points)):
            x, y, _ = points[i]
            # y, _, x = points[i] # lidar coords
            if x > -3 and x < 3 and y > -1.5 and y < 1.5:
                    mask[i] = False
    # print(points)
    # print(mask)
...
def compute_3d_bounding_boxes(cluster_labels, point_cloud):
    ...
    for label in unique_labels:
        ...
        cluster_points = np.array(point_cloud.points)[(cluster_labels == label)]
